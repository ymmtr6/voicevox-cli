...
### `speak-hooks` — Stop hook 用読み上げ

Claude Code の Stop hook から stdin で受け取った JSON を解析し、最後の assistant メッセージを読み上げます。
...
- `transcript_path` から最後の assistant メッセージを取得して読み上げ
- transcript がない場合はフォールバックメッセージを読み上げ

| オプション | 説明 | デフォルト |
|---|---|---|
| `-s, --speaker <id>` | 話者 ID | (設定ファイル/環境変数) |
| `--speed <speed>` | 話速 | (設定ファイル/環境変数) |
| `--chars <n>` | 読み上げ文字数の上限 | `100` |
...
- `stop_hook_active: true` のときはスキップ（無限ループ防止）
- `transcript_path` から最後の assistant メッセージの先頭 100 文字を読み上げ
- transcript がない場合は `"クロードの作業が完了しました"` を読み上げ

読み上げ文字数やフォールバックメッセージはオプションで変更できます。

```json
{
  "command": "voicevox-cli speak-hooks --chars 50 --fallback 完了しました"
}
```
